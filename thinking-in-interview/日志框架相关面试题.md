...
#### 2.配置多个AsyncLogger会创建出多个disruptor实例吗？
...
由上面的源码可以得出`RingBuffer Size`如果是web项目为`262144 byte (256kb)`，否则`4096 byte (4kb)`。

如果需要修改此值，则我们需要需要在环境中添加配置文件`log4j2.component.properties`，并且设置`AsyncLoggerConfig.RingBufferSize=n`来直接指定`RingBuffer Size`的大小。

#### 3.RingBuffer满了log4j2会怎么办？
...
#### 4.EventProcessor线程的等待策略常用的有哪几种？
...
#### 6.如何大量的异常信息导致日志性能低下，占用CPU资源和IO资源？
...
#### 7.如何防止RingBuffer满了造成线程BLOCK，进而内存占用飙升，大量挂起线程，系统停顿？
