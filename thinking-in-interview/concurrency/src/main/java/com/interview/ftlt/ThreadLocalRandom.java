...
public final class ThreadLocalRandom extends Random {
    ...
    // 下面这段代码是为了防止伪共享。通常 CPU 的缓存行一般是 64 或 128 字节，
    // 为了防止不同实例被加载到同一个缓存行，我们需要多余填充一些字段，使得每个实例的大小超出缓存行的大小。
    // JDK8之后其实可以直接使用自带的ThreadLocalRandom了，因为JDK8在Thread中维护了种子变量，同时利用了@Contended注解解决了缓存行的伪共享问题
    // 这里有一个疑问，为什么要将random的种子变量维护在Thread对象中呢？ 对时间（访问速度更快）和空间（占用内存更小）的更优考虑。
    // 三个字段比装箱到填充的 ThreadLocal 对象需要更少的空间。比直接通过 ThreadLocal 来访问要快速得多。
    // 参考地址 https://stackoverflow.com/questions/34529186/new-additional-fields-in-java-lang-thread-what-is-the-idea
