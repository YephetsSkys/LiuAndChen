> 开发人员必备的9大MySQL索引和查询优化一般来说，程序员的面试内容分为两部分，一部分与编程相关，另一部分则与数据库相关。而作为数据库中的主流，MySQL更是涉及面试中的诸多高频考点。对于后端人员来说，不需要像专业的DBA那样精通MySQL，但也需要掌握相关的基本内容。根据头条上总结的面试题目，回答并记录在此。
...
- 注意不通事务间表的访问顺序，减少因为顺序不一致造成的死锁现象；
- 数据库读写分离；
- 将大事务分成多个小型事务；
